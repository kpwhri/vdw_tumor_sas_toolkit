...
%macro ReadNAACCRv18(input_file,output_dset);
    *this macro reads a NAACCR v18 filename reference and returns a SAS dataset with XML/NAACCR ID names. ;
    %let nogo = 0;
    %put INFO: this macro reads a NAACCR v18 filename reference and returns a SAS dataset with XML/NAACCR ID names. Works with NAACCR v18. Not compatible with XML format.;
    %if %length(&input_file.) = 0 %then %do;
        %put INFO: No input file specified.;
        %let nogo = 1;
    %end;

    %if %length(&output_dset.) = 0 %then %do;
        %put INFO: No output dataset specified;
        %let nogo = 1;
    %end;
    ...
    %if &nogo = 0 %then %do;
        data &output_dset.;  
            ...
            * infile inv18 lrecl =  6934 pad missover; /* CONFIDENTIAL */
              infile &input_file. lrecl = 24194 pad missover; /* TEXT */
    ...
    %end;
    %else %do;
        %put WARNING: Input/Output not specified. Nothing done.;
    %end;
%mend ReadNAACCRv18;

%macro WriteNAACCRv18();
    data _NULL_;
       set SAStest;
